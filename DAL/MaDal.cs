...
public class MaDal : IdentityDbContext<AccountDAO, RoleDAO, Guid>
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        var ac4 = new AccountDAO() { UserName = "Dom" };

       

        modelBuilder.Entity<AccountDAO>().HasData(new List<AccountDAO>() { ac1, ac2, ac3, ac4 });
        ...
        var u3 = new AppUserDAO() { IdUser = ac3.Id, FirstName = "Benoit", LastName = "Saby", PhoneNumber = "054123845", City = "Embrun" };
         var u4 = new AppUserDAO() { IdUser = ac4.Id, FirstName = "Dom", LastName = "czdcz", PhoneNumber = "05412456895", City = "Pessac" };




        modelBuilder.Entity<AppUserDAO>().HasData(new List<AppUserDAO>() { u1, u2, u3,u4 });
...
{
    var userone= new AccountDAO(){UserName="Dom"};
    userone.SecurityStamp=DateTime.Now.ToLongDateString();
    
    var passwordHasher = new  PasswordHasher<AccountDAO>();
    // j'enregistre le hash du mot de passe dans boss
    var hashed = passwordHasher.HashPassword(userone, "123ABCdef*");
    userone.PasswordHash = hashed;
    modelBuilder.Entity<AccountDAO>().HasData(userone);
